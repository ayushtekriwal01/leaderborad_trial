...
const ALIASES = {
  ...
  orders: ["orders", "total_orders", "total__orders"],
  gmv: ["gmv", "total_gmv", "total__gmv"],
  region: ["region", "dim__creator_region"],
...
// Deterministic ordering: NMV desc → Orders desc → creator_id asc
...
export function normalizeRows(rows) {
  ...
  for (const raw of rows) {
    ...
    const c = {
      ...
      nmv,
      orders: toNum(pick(raw, ALIASES.orders)) ?? 0,
      gmv: toNum(pick(raw, ALIASES.gmv)) ?? 0,
      region: normRegion(pick(raw, ALIASES.region)),
      state: pick(raw, ALIASES.state) ? String(pick(raw, ALIASES.state)).trim() : null,
    };
  ...
  return { creators: [...byId.values()], dropped };
}

export function buildDataset(rows, { version, generatedAt } = {}) {
  const { creators, dropped } = normalizeRows(rows);

  // Classify (mutually exclusive by construction) + rank
  for (const c of creators) c.cohort = classifyCohort(c.nmv);
  creators.sort(creatorSort);
  creators.forEach((c, i) => (c.overallRank = i + 1)); // eligible universe only (<15K already excluded)

  for (const key of Object.keys(COHORTS)) {
    const pool = creators.filter((c) => c.cohort === key); // already sorted
    pool.forEach((c, i) => (c.nationalRank = i + 1));
    for (const region of REGIONS) {
      const rp = pool.filter((c) => c.region === region);
      rp.forEach((c, i) => (c.regionalRank = i + 1));
    }
  }
  ...
  for (const [key, def] of Object.entries(COHORTS)) {
    const pool = creators.filter((c) => c.cohort === key);
    cohorts[key] = {
      label: def.label,
      total: pool.length,
  ...
  const dataset = {
    ...
    counts: { eligible: creators.length, dropped },
    cohorts,
