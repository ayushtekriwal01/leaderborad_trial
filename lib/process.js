...
export function buildDataset(rows, { version, generatedAt, windowLabel } = {}) {
  ...
  // OVERALL board — the entire eligible universe, all cohorts together.
  // National = top of the overall ranking; regional = rank within region across ALL cohorts.
  const overall = {
    label: "Overall",
    total: creators.length,
    national: lb(creators.slice(0, NATIONAL_TOP_N)).map((r) => ({ ...r, nationalRank: r.overallRank })),
    regions: Object.fromEntries(
      REGIONS.map((reg) => [
        reg,
        lb(creators.filter((c) => c.region === reg).slice(0, REGIONAL_TOP_N)).map((r, i) => ({
          ...r,
          regionalRank: i + 1,
          nationalRank: r.overallRank,
        })),
      ])
    ),
  };
...
export function validateDataset(ds) {
  ...
  // Overall (all-cohorts) boards
  if (ds.overall) {
    checkBoard("overall/national", ds.overall.national, "nationalRank", NATIONAL_TOP_N);
    for (const region of REGIONS) {
      const ob = ds.overall.regions[region];
      checkBoard(`overall/${region}`, ob, "regionalRank", REGIONAL_TOP_N);
      for (let i = 1; i < ob.length; i++) {
        if (ob[i].overallRank <= ob[i - 1].overallRank)
          err(`overall/${region}: overall-rank order broken at position ${i + 1}`);
      }
      for (const r of ob) if (r.region !== region) err(`overall/${region}: creator ${r.id} region mismatch`);
    }
  }
